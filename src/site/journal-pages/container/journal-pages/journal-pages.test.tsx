...
import { JournalPages } from './journal-pages';
...
const initProps: any = {
  ...
  loadPortalJournalPages: jest.fn(),
  loadPortalJournalPagesSuccess: jest.fn(),
  loadPortalJournalPagesFailure: jest.fn(),
  user: { name: 'test name' },
};
...
jest.mock('../../services/journal-pages.service', () => ({
  createPortalJournalPage: jest
    .fn()
...
function setup(props: any): any {
  const enzymeWrapper: ShallowWrapper = shallow(<JournalPages {...props} />);
...
describe('Pages', () => {
  ...
  it('should call for page list success on init', async () => {
    ...
    props.loadPortalJournalPagesSuccess.mockClear();
    const flushPromises = () => new Promise(setImmediate);
    await flushPromises();
    expect(props.loadPortalJournalPagesSuccess.mock.calls.length).toBe(1);
    expect(props.loadPortalJournalPagesSuccess.mock.calls[0][0]).toEqual([
  ...
  it('should call for page list failure on init', async () => {
    jest.spyOn(React, 'useEffect').mockImplementationOnce((f: any) => f());
    const { props } = setup(initProps);
    props.loadPortalJournalPagesFailure.mockClear();
    const flushPromises = () => new Promise(setImmediate);
    await flushPromises();
    expect(props.loadPortalJournalPagesFailure.mock.calls.length).toBe(1);
    expect(props.loadPortalJournalPagesFailure.mock.calls[0][0]).toEqual(
      'error'
    );
  });
