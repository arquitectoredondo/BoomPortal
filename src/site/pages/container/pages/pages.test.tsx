...
const initProps: any = {
  ...
  loadPortalPages: jest.fn(),
  loadPortalPagesSuccess: jest.fn(),
  loadPortalPagesFailure: jest.fn(),
  user: { name: 'name test' },
};
...
function setup(props: any): any {
  const enzymeWrapper: ShallowWrapper = shallow(<Pages {...props} />);
...
describe('Pages', () => {
  ...
  it('should call for page list success on init', async () => {
    ...
    props.loadPortalPagesSuccess.mockClear();
    const flushPromises = () => new Promise(setImmediate);
    await flushPromises();
    expect(props.loadPortalPagesSuccess.mock.calls.length).toBe(1);
    expect(props.loadPortalPagesSuccess.mock.calls[0][0]).toEqual([
  ...
  it('should call for page list failure on init', async () => {
    jest.spyOn(React, 'useEffect').mockImplementationOnce((f: any) => f());
    const { props } = setup(initProps);
    props.loadPortalPagesFailure.mockClear();
    const flushPromises = () => new Promise(setImmediate);
    await flushPromises();
    expect(props.loadPortalPagesFailure.mock.calls.length).toBe(1);
    expect(props.loadPortalPagesFailure.mock.calls[0][0]).toEqual('error');
  });
