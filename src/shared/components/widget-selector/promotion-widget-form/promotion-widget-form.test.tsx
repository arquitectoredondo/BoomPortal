...
import PromotionWidgetForm from './promotion-widget-form';
...
const initProps: any = {
  ...
  onSave: jest.fn(),
  onCancel: jest.fn(),
  promotionWidgetData: mockBanner,
};
...
describe('PromotionWidgetForm', () => {
  ...
  it('should call for portal settings success on init', async () => {
    jest.spyOn(React, 'useEffect').mockImplementationOnce((f: any) => f());
    setup({ ...initProps, promotionWidgetData: undefined });
    const flushPromises = () => new Promise(setImmediate);
    await flushPromises();
  });
  ...
  it('should handle close dialog saving', () => {
    ...
    const form: ShallowWrapper = enzymeWrapper.find(".widget-promotion-body");
